...
export default function ClearancePage() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      setRequests(data);
    } catch { toast.error('Failed to load clearance requests'); }
  ...
  const handleReview = async (id: number, status: 'APPROVED' | 'REJECTED') => {
    try {
      if (status === 'APPROVED') await approveClearance(id);
      else await rejectClearance(id);
